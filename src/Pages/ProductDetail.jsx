...
const ProjectDetail = () => {
    ...
    const cleanHTML = (html) => {
        ...
        return html
            .replace(/\n/g, " ")                // remove xuống dòng
            .replace(/>\s+</g, "><")            // remove space giữa tag
            .replace(/\s+/g, " ")               // gom space
            .replace(/&nbsp;/g, " ")            // fix nbsp
            .replace(/(<br\s*\/?>\s*){2,}/gi, "<br/>"); // fix spam br
    };
        ...
        <div className="bg-slate-50/30 min-h-screen w-full">
            ...
            <nav className="w-full sticky top-0 bg-white/80 backdrop-blur-md z-[100] border-b border-slate-100">
                <div className="max-w-5xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center">
                    <Link to="/" className="flex items-center gap-2 text-slate-500 hover:text-blue-600 font-bold text-xs sm:text-sm transition-all">
            ...
            {/* MAIN */}
            <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12 md:py-20 bg-white shadow-sm border-x border-slate-50 min-h-screen">
